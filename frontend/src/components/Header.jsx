    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${background} ${textColor} ${headerPadding} ${headerShadow}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          {/* Desktop Navigation: visible only from lg and up */}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className={`lg:hidden fixed top-[72px] left-0 right-0 max-h-screen overflow-y-auto px-4 py-4 z-40 ${isDark ? 'bg-gray-900 text-white' : 'bg-white text-gray-800'} shadow-xl rounded-b-lg`}
          >
            {['Home', 'Features', 'Testimonials'].map((text) => (
              <Link
                key={text}
